...
def apply_patches(split_dir, patch_dir, split_names=['train', 'dev', 'test']):
    ...
    for name in split_names:
        # Set paths
        split_path = split_dir + name + '.json'
        patch_path = patch_dir + name + '_id2label.json'
        # Load data
...
def save_patches(patch2data, save_dir):
    for patch_name, data in patch2data.items():
        save_path = save_dir + patch_name + '.json'
        save_json(data, save_path)


if __name__ == '__main__':
    tacred_dir = '../tacred/'  # Directory where TACRED is stored
    patch_dir = './'  # Directory where patches are located
    save_dir = './'  # Directory where patched data should be saved
