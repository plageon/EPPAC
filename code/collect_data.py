...
class EntityPromptDataset(DictDataset):
    ...
    def __init__(self, train, predict=None, subj_obj_pair=None, path=None, name=None, tokenizer=None,
                 virtual_prompt=None,
                 features=None, rel_length=None, max_seq_length=512):
    ...
    @classmethod
    def load(cls, train, predict=None, subj_obj_pair=None, path=None, name=None, virtual_prompt=None, tokenizer=None):
        path = path + "/" + name + "/"
        features = {}
        ...
        res = cls(train=train, predict=predict, subj_obj_pair=subj_obj_pair, virtual_prompt=virtual_prompt,
                  features=features,
                  tokenizer=tokenizer)
        return res
    ...
    def list2tensor(self, data, tokenizer):
        ...
        for index, item in enumerate(tqdm(data)):
            ...
            if self.train and self.predict == 'rel' and not self.subj_obj_pair == "None" and not subj_obj_pair == self.subj_obj_pair:
                continue
            ...
            if self.predict == "subj":
                ...
                if self.subj_obj_pair == "None":  # None means the model is training with single level
                    label = self.virtual_prompt.rel2id[item['relation']]
                else:  # double level
                    if item['relation'] not in self.virtual_prompt.subj_obj_pair2rel2id[self.subj_obj_pair]:
                        label = -1  # there is no way the model can make a right prediction
                    else:
            ...
            res['mlm_labels'].append(mlm_labels)  # <mask>
    ...
    def tokenize(self, item, tokenizer):
        ...
        #build prompt for different prediction type
        if self.predict == "subj":
        ...
        input_ids = tokenizer.build_inputs_with_special_tokens(tokens)  #add special tokens:[cls][sep]
        token_type_ids = tokenizer.create_token_type_ids_from_sequences(tokens)  # token type
