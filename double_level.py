...
from code.collect_data import EntityPromptDataset, collect_raw_data
from code.optimizing import get_optimizer, get_optimizer4temp, get_optimizer4dense,get_optimizer4norm
from code.utils import get_tokenizer_mlm, set_seed, get_raw_temps, evaluate, f1_score
...
def train_sub_model(model_scale=None, dataset_name=None, predict=None, subj_obj_pair_id=None, reload_data=True,
                    train=True, vec_sim=None, subj_length=None, obj_length=None, rel_length=None, random_centers=False, fixed_centers=False,fixed_LM=False,
    ...
    #initialize template
    raw_temp = None if random_centers else get_raw_temps(data_dir)
    virtual_prompt = VirutalPrompt(raw_temp=raw_temp, tokenizer=tokenizer, mlm=mlm, subj_length=subj_length, obj_length=obj_length, rel_length=rel_length,
                                   datadir=data_dir, random_centers=random_centers)
    ...
    print(" time=", datetime.datetime.now(),
          ...
          " subject_length=", subj_length,
          " object_length=", obj_length,
          ' relation length=', rel_length,
          " random_center=", random_centers,
    ...
    optimizer4temp, scheduler4temp = get_optimizer4dense(model, lr_temp) if vec_sim == "dense" else get_optimizer4temp(
        model, lr_temp)
    optimizer4norm, scheduler4norm = get_optimizer4norm(model, lr_temp)
    ...
    if load_kpl:#load history model
    ...
    if train:
        for epoch in trange(int(num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                loss = criterion(logits, batch['labels'])  # calculate loss
                ...
                if (step + 1) % gradient_accumulation_steps == 0:
                    ...
                    if not fixed_LM: #do not finetune LM parameters
                        optimizer.step()
                        scheduler.step()
                    if not fixed_centers: #do not optimize templates
                        optimizer4temp.step()
                        scheduler4temp.step()
                    optimizer4norm.step()
                    scheduler4norm.step()
...
def evaluate_sub_model(vec_sim, subj_length, obj_length, rel_length, model_scale, dataset_name, subj_model_path, obj_model_path, rel_model_path,
                       eval_set):
    model_type = 'roberta'
    model_name = 'roberta-' + model_scale
    tokenizer, mlm = get_tokenizer_mlm(model_type, model_name)
    ...
    raw_temp = None if random_centers else get_raw_temps(data_dir)
    virtual_prompt = VirutalPrompt(raw_temp=raw_temp, tokenizer=tokenizer, mlm=mlm, subj_length=subj_length,obj_length=obj_length, rel_length=rel_length,
                                   datadir=data_dir,random_centers=True)
...
if __name__ == "__main__":
    LEARNING_RATE = {#learning rate for PLM parameters and template parameters
        "subj": [1e-6, 1e-6],
        "obj": [1e-5, 1e-5],
        "rel": {
    ...
    SUBJ = True #train subj type classifier
    OBJ = True #train obj type classifier
    REL = True #train specialized relation classifier
    EVAL = True #evaluate model
    model_scale = "base" #roberta-base/roberta-large
    dataset_name = "tacred"
    vec_sim = "cdist2" #vector similarity ["mm","cos"，“dense","cdist1","cdist2"]
    subj_length=5 #template length for subject type classifier
    obj_length=5
    rel_length = 5
    random_centers = True #use randomly initialized templates
    fixed_centers = False #free template parameters
    fixed_LM=False #freese PLM parameters
    num_epoch = 1
    eval_set = "test" #use test/dev segment to evaluate
    model_path = "./results/" + dataset_name + "/best_models.txt"
    ...
    if SUBJ:
        ...
        subj_model = train_sub_model(vec_sim=vec_sim,subj_length=subj_length,obj_length=obj_length, rel_length=rel_length, model_scale=model_scale,
                                     dataset_name=dataset_name, predict='subj',
                                     learning_rate=learning_rate, lr_temp=lr_temp, num_train_epochs=num_epoch * 2,
                                     random_centers=random_centers, fixed_centers=fixed_centers,fixed_LM=fixed_LM, )
    ...
    if OBJ:
        ...
        obj_model = train_sub_model(vec_sim=vec_sim,subj_length=subj_length,obj_length=obj_length, rel_length=rel_length, model_scale=model_scale,
    ...
    if REL:
        ...
        for subj_obj_pair_id in range(0, 27):
            ...
            rel_model = train_sub_model(vec_sim=vec_sim,subj_length=subj_length,obj_length=obj_length, rel_length=rel_length, model_scale=model_scale,
                                        dataset_name=dataset_name, predict='rel',
                                        subj_obj_pair_id=subj_obj_pair_id, learning_rate=learning_rate, lr_temp=lr_temp,
                                        num_train_epochs=num_epoch * 8, random_centers=random_centers,
                                        fixed_centers=fixed_centers,fixed_LM=fixed_LM, )
    ...
    if EVAL:
        ...
        with open(model_path, 'r', encoding='utf-8') as f:
            for line in f.readlines():
                ...
                if len(line) > 0:
                    models.append(line)
        evaluate_sub_model(vec_sim,subj_length,obj_length, rel_length, model_scale, dataset_name, models[0], models[1], models[2:], eval_set)
